...
//   - `NAME=value` at top level → globals entry
...
// section is one of {globals, command-NAME, main-command}.
type section struct {
...
func Translate(bashSrc, programName string) string {
	...
	for idx := 0; idx < len(lines); idx++ {
		...
		// NAME=VALUE — global at top level
		if cur == nil {
...
// assemble glues prologue + globals + commands into the final .perch text.
