...
- **`exec` keyword dropped from idiomatic use — a bare declared bin IS the call.** `docker ps`, `git commit -m "x"`, `go build` run the declared binary directly; the `exec` keyword is now the *explicit* form, needed only when the bin name collides with a built-in op (`exec rm`, `exec mkdir`, `exec chmod`) or inside a `let` capture (`let h = exec git rev-parse HEAD`). All recipes/demos/docs converted to the bare form; the `exec` op/grammar stays as the escape hatch.
