...
- **Subprocesses no longer inherit the full host environment — env is scrubbed to the manifest (security).** A declared bin (`docker`, `git`) used to inherit *every* env var, so a spawned process could read `AWS_SECRET_KEY` / `GITHUB_TOKEN` even though perch's own ops were gated. Now a subprocess sees only: a **default operational set** (`PATH`, `HOME`, `TMPDIR`, `LANG`, … — the OS plumbing tools need, declared once for you), env vars the file declared via `requires env "NAME"`, names the operator allowed via `--env`, and the program's own bindings + per-command `env`. Everything else is dropped — a spawned tool literally cannot read a secret the file didn't declare. Scrubbing is on whenever a `requires` block is present (a missing block normalizes to an empty manifest, so it's on by default). **Scope:** this closes the env-var (`vars`) leak; a declared subprocess's *filesystem* and *network* access is still bounded only by the OS, not by `requires read/write/host` — kernel-level confinement (sandbox-exec / Landlock) is future work, documented in [sandboxed-by-design.md](docs/sandboxed-by-design.md).
- **No `let` keyword — `=` is the universal assignment operator (breaking).** A capture is written `head = git rev-parse HEAD`, `size = file_size "./bin"`, `out = pipe … end` — no `let`. The same `=` declares a shared binding at file scope (`BUILD_DIR = "./out"`) and a capture inside `do`; the loader routes by context (top level → binding, inside `do` → capture, resolved via the bare-dispatch pipeline). Var-ref ergonomics for a single bare-ident arg (`shout = upper who`) and pipe-block captures (`out = pipe … end`) carry over. All recipes/demos/docs/tests converted; an old `let NAME = …` line now raises a clear migration error. Grammar: the `let_*` forms are replaced by `assign_*` (no `let` literal); the LSP / tree-sitter / VS Code / docs keyword sets drop `let`.
- **Quotes are optional on every scalar value.** A bare token works anywhere a quoted string did — `name redis`, `version 1.0.0`, `bin docker`, `env HOME`, `host api.github.com`, `read ./config`, `description Start the redis container`, `default 6379`, `print hello`, `BUILD_DIR = ./out`. Quote only when the value contains spaces or a character that would otherwise tokenize (`about "Manage things"`). A bare and a quoted program parse to the same result and run identically. Mechanically: scalar captures use capy's `word` capture (accepts bare words, numbers, dotted versions like `1.0.0`, and quoted strings) and emit through `asString` (normalizes both forms to one JSON string). Because every value is normalized to a string, the type of a `default`/binding is recovered by parsing (`6379`→int, `true`→bool) — so `n = 42` and `n = "42"` are now indistinguishable. Pinned by `TestQuotesOptional_*`.
- **`exec` keyword dropped from idiomatic use — a bare declared bin IS the call, captures included.** `docker ps`, `git commit -m "x"`, `go build` run the declared binary directly, and so do captures: `let head = git rev-parse HEAD`, `let ctx = kubectl config current-context`. The `exec` keyword is now the *explicit* form, needed only when the bin name collides with a built-in op (`exec rm`, `exec mkdir`, `exec chmod`). All recipes/demos/docs converted to the bare form; the `exec` op/grammar stays as the escape hatch.
- **Unified `let` capture grammar — no arity ladder (breaking internals).** Every bare capture (`let s = sha256 "x"` op, `let r = docker ps -q` bin, `let size = file_size "x"` value-op) parses through one path: argv as a quote-preserving tail, op-vs-bin decided at **load** time. The old per-arity op-capture forms (which emitted malformed JSON for bare/flag tokens like `mktemp -d` or `kubectl config current-context`) are gone. The loader carries a canonical op-kind list (`infra/capyloader/opkinds.txt`, kept in sync with the handler registry by a drift test) to fold a captured-exec back into a native op-capture when the leading name is a built-in op. A single bare-ident argument keeps its var-ref ergonomics — in captures (`let shout = upper who`) **and** in flat statements (`ensure_dir BUILD_DIR` resolves the `BUILD_DIR` binding, mirroring `write BUILD_DIR` in `requires`). Declared bins stay subprocesses and remain gated by `requires`.
- **Op args behave like CLI tokens, in every position.** Each argv token is classified independently: a bare identifier resolves to a binding of that name (else stays its literal text — `git rev-parse HEAD`, `replace text foo bar`), a quoted token is literal even with embedded spaces, `${x}` interpolates. This now works for *all* positions and arities (`path_join SRC DST` → both resolve; `format "x %s" "y"` keeps `"x %s"` as one token), not just the single-arg case. A **declared bin** keeps literal argv — a bare token matching a binding is the subprocess's own argument, not a lookup (`echo SRC` prints `SRC`). Fixes a latent quote-stripping bug where a quoted multi-word argv token (`docker ps --format "{{.Status}}  {{.Ports}}"`) was mis-split; the argv tail is now captured quote-preserving via `toJSON`.
- **No `run` / `call` keywords — bare-name dispatch (breaking).** A bare leading name now resolves against a single registry: a built-in op runs, a **command** is invoked (`deploy -target=linux`), a **template** is expanded (`ensure_dir "./out"`), a declared **bin** is spawned (`docker ps`). `run`/`call` were removed; `exec NAME` still forces the subprocess reading. The loader's `resolveBareDispatch` rewrites an implicit-exec op whose name is a command/template into the right op before template expansion.
- **Unique-name registry — error on duplicates (breaking).** Because bare names dispatch, every dispatchable name must be distinct: a command/template may not shadow a built-in op/keyword, and command/template/bin-alias names may not collide. `perch --check` errors on any clash. (Forced a few recipe renames: `shell`→`console`, `test`→`gotest`, `run`→`launch`, `open`→`browse`, `which`→`paths`.)
- **No `globals … end` block — bare top-level bindings (breaking).** Shared bindings are declared bare at file scope (`BUILD_DIR = "${script_dir}/.out"`); a leftover `globals` block is a clear load error. Bindings can be referenced in `requires` by name — `write BUILD_DIR` desugars to `write "${BUILD_DIR}"`.
- **`bin` paths + `as NAME` handles.** A `bin` may be a path to an executable (`bin "./bins/tool.exe"`, checked on disk relative to the `.perch` file), and `bin "PATH" as NAME` gives it a clean handle invoked by name (`tool …`), resolved to the path before spawning. Both the alias and the path satisfy the capability gate.
- **`exec` chaining — `&&` / `||` / `;`.** `exec a && exec b`, `exec a || exec b`, `exec a ; exec b` join clauses by exit status (perch operators evaluated on each clause's exit code, never handed to a shell). The loader folds an operator-containing exec line into an `exec_chain` block op; operators are literal source tokens so the §3.3 keystone holds (a `${x}` value can't become an operator). Short-circuits; the chain raises if its last actually-run clause fails.
- **`shell` is deprecated in favor of `exec`.** `shell`/`shell_output` are marked deprecated in the op reference and docs; `exec` (+ `pipe`, `with_env`, the line-toolbox, and `&&`/`||`/`;`) is the recommended shell-free default. `shell` still works — keep it only for genuine shell needs (a value that must word-split like `${proxy_args}`, or a one-off `awk`/`sed` chain). All demos converted to `exec`.

- **Adopted capy `5102dec` (the all-fixes engine build).** Bumped `github.com/olivierdevelops/capy` to `v0.20.1-0.20260529061856-5102decfe5d0`, which lands deterministic candidate ordering, block-opener backtracking, `word` / `asString` captures, indent lookahead (`when_followed_by` / `when_not_followed_by indent`), and `comments`. All additive — the existing grammar and suite are unaffected. See [docs/capy-limitations.md](docs/capy-limitations.md) for the full before/after.
...
- DSL defined entirely via [capy](https://olivierdevelops.github.io/capy) (`infra/capyloader/lib.capy`).
- Four CLI modes:
