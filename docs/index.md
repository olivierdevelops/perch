...
🔢 <strong>Version extract + compare ops</strong> — <code>let v = version_extract "${shell_output_of_kubectl_version}"</code> then <code>version_ge "${v}" "1.28.0"</code>. No per-binary parsers; user controls the regex (with a sensible default), perch handles the numeric ordering. <code>~=</code>-style compatibility via <code>version_compat</code>. ·
