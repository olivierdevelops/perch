...
| `exec BIN tok…`          | same | **Explicit form** of a bare bin call. Needed only when the bin name collides with a built-in op (`exec rm`, `exec mkdir`, `exec chmod`) or inside a `let` capture (`let h = exec git rev-parse HEAD`). See [sandboxed-by-design.md §3.2](sandboxed-by-design.md). |
